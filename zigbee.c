...
static uint32_t zigbee_encode_ATcmd(uint8_t* buffer, uint32_t size, uint8_t frameID, const char* atCmd, uint8_t* data, uint32_t dataSize)
{
  uint32_t i;
  uint32_t sizeFrame;
  sizeFrame  = 0;
...
//AT NK
uint32_t zigbee_encode_setNetworkEncryptionKey(uint8_t* buffer, uint32_t size, uint8_t frameID, zigbee_encryptionKey* key)
{
  return 
    zigbee_encode_ATcmd(buffer, size, 
                        frameID,
...
uint32_t zigbee_encode_transmitRequest(uint8_t* buffer, uint32_t sizeBuffer, uint8_t frameID, zigbee_64bDestAddr* destAddr64b, uint16_t destAddr16b, uint8_t* payload, uint8_t size)
{
  uint32_t sizeFrame;
  uint32_t index;
  sizeFrame  = 0;
