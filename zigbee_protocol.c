...
static zb_status zigbee_protocol_enableEncryption(zigbee_obj* obj, zigbee_encryptionKey* encryptKey, zigbee_linkKey* linkKey);
static bool      zigbee_protocol_waitAndcheckReply(uint32_t fd, uint8_t* receivedFrame, uint32_t sizeBuffer, zigbee_decodedFrame* decodedData);
...
zb_status zigbee_protocol_sendData(zigbee_obj* obj, zigbee_64bDestAddr* destAddr64b, uint16_t destAddr16b, uint8_t* payload, uint8_t size)
{
  zb_handle_status handle_status;
  zb_status status;
  ...
  zigbee_protocol_incrementFrameID(obj);
  obj->sizeOfFrameToSend = zigbee_encode_transmitRequest(obj->frame, obj->frameSize, obj->frameID, destAddr64b, destAddr16b, payload, size);
