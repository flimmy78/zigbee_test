...
//  status = zigbee_protocol_setBaudRate(&zigbee, 115200);
  ...
  if (status == 0)
  {
    status = zigbee_protocol_setNodeIdentifier(&zigbee, "ENDD");
  }
  /*
  if (status == 0)
  {
    status = zigbee_protocol_setSleepMode(&zigbee, PIN_SLEEP);
  }
  
  if (status == 0)
  {
    status = zigbee_protocol_configureIO(&zigbee);
  }
*/
...
retry:
  //  fprintf(stdout, "Network activated, wait for end of scanning\n");
    ...
    fprintf(stdout, "Network assocation activated status = 0x%x, joining status = 0x%x\n", status, indicationStatus);
    //status = 0; indicationStatus = 0;
    if ((status == 0) && ((indicationStatus == 0x25) || (indicationStatus == 0x21) || (indicationStatus == 0x22) ) && (nbRetry < 10))
    {
      nbRetry++;
      sleep(2);
      goto retry;
  ...
  if ((status == 0) && (indicationStatus == 0))
  {
    ...
    zigbee_64bDestAddr coordinator64bAddr = { 0, 0, 0, 0, 0, 0, 0, 0 } ;
    uint8_t* payload = (uint8_t*) "I'm alive!"; 
    sleep(5);
    status = zigbee_protocol_sendData(&zigbee, &coordinator64bAddr, ZIGBEE_UNKNOWN_16B_ADDR, payload, strlen((char*) payload)+1);
    ...
    while (1)
    {
      ...
      zigbee_protocol_sendData(&zigbee, &coordinator64bAddr, ZIGBEE_UNKNOWN_16B_ADDR, (uint8_t*) data, strlen(data)+1);
#else
      while (1);
#endif
    }
